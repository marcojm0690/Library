...
# Optional app settings (storage and vision)
STORAGE_ACCOUNT_NAME="${STORAGE_ACCOUNT_NAME:-}"
STORAGE_CONTAINER_NAME="${STORAGE_CONTAINER_NAME:-user-libraries}"
...
# Grant Storage Blob Data Contributor role (if storage account provided)
if [ -n "$STORAGE_ACCOUNT_NAME" ]; then
  echo "Granting Storage Blob Data Contributor role..."
  STORAGE_ID=$(az storage account show -n "$STORAGE_ACCOUNT_NAME" -g "$RG" --query id -o tsv 2>/dev/null || echo "")
...
# Grant Cognitive Services User role (if vision endpoint provided)
...
APP_SETTINGS="WEBSITES_PORT=8080 ASPNETCORE_ENVIRONMENT=Production Azure__CosmosDb__Endpoint=${COSMOS_ENDPOINT} Azure__CosmosDb__DatabaseName=${COSMOS_DB_NAME} Azure__CosmosDb__ContainerName=${COSMOS_CONTAINER_NAME}"

# Add optional storage settings if provided
if [ -n "$STORAGE_ACCOUNT_NAME" ]; then
  APP_SETTINGS="$APP_SETTINGS Azure__Storage__AccountName=${STORAGE_ACCOUNT_NAME} Azure__Storage__ContainerName=${STORAGE_CONTAINER_NAME}"
fi

# Add optional vision settings if provided
if [ -n "$VISION_ENDPOINT" ]; then
  APP_SETTINGS="$APP_SETTINGS Azure__Vision__Endpoint=${VISION_ENDPOINT}"
fi

az webapp config appsettings set -g "$RG" -n "$WEBAPP_NAME" --settings $APP_SETTINGS --only-show-errors
